...
from data.ferplus.dataset_ferplus import FERPlusDataSet
from torchvision import datasets
from sklearn.metrics import f1_score, confusion_matrix
from time import time
from utils import *
from data_preprocessing.sam import SAM
from models.S2BF import S2BF
...
def run_training():
    ...
    if args.dataset == "rafdb":
        ...
        model = S2BF(img_size=224, num_classes=num_classes, type=args.modeltype)
        ...
        datapath = './data/ferplus/'
        num_classes = 8
        train_dataset = FERPlusDataSet(datapath, phase='train', transform=train_transform)
        val_dataset = FERPlusDataSet(datapath, phase='test', transform=test_transform)
        model = S2BF(img_size=224, num_classes=num_classes, type=args.modeltype)

    elif args.dataset == "caer":
        traindir = './data/CAER-S/train'
        valdir = './data/CAER-S/test'
        num_classes = 7
    ...
    for i in range(1, args.epochs + 1):
        ...
        print(
            '[Epoch %d] Train time:%.2f, Training accuracy:%.4f. Loss: %.3f LR:%.6f'
        ...
        with torch.no_grad():
            ...
            print(
                "[Epoch %d] Validation accuracy:%.4f, Loss:%.3f, f1 %4f, score %4f"
                % (i, val_acc, val_loss, f1, total_socre)
            )

            if val_acc > 0.6395 and val_acc > best_acc:
                torch.save(
                    {
                        'iter': i,
                    ...
                    os.path.join(
                        './checkpoint', "epoch" + str(i) + "_acc" + str(val_acc) + ".pth"
                    ),
                )
                print('Model saved.')
            if val_acc > best_acc:
